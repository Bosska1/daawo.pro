...
import { Home, PlayCircle, Tv, Heart, Settings } from 'lucide-react';
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-gray-950/95 backdrop-blur-md border-t border-gray-800 p-2 z-20 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.2)] navbar">
      ...
      <Tabs defaultValue={getActiveTab()} value={getActiveTab()} className="w-full relative z-10">
        <TabsList className="w-full bg-gray-900 grid grid-cols-5 h-auto p-1 rounded-xl">
          <TabsTrigger 
            value="/" 
            asChild 
          ...
          <TabsTrigger 
            ...
            className="flex-1 data-[state=active]:bg-gray-800 data-[state=active]:text-primary data-[state=active]:shadow-[0_0_10px_rgba(56,239,125,0.3)] rounded-lg"
          >
            <Link to="/admin" className="flex flex-col items-center py-1.5">
